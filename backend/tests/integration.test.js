...
describe('Integration tests - CRUD with real DB (in-memory)', () => {
  it('should create a board, column and card, then perform updates and deletions', async () => {
    // Create board
    let res = await request(app).post('/api/boards').send({ name: 'IntegrationBoard' })
    expect(res.status).toBe(201)
    const board = res.body
    expect(board).toHaveProperty('_id')

    // Create column
    res = await request(app).post(`/api/boards/${board._id}/columns`).send({ key: 'col-key', title: 'Backlog' })
    expect(res.status).toBe(201)
    const column = res.body
    expect(column).toHaveProperty('_id')

    // Create card
    res = await request(app).post(`/api/boards/${board._id}/cards`).send({ title: 'Ticket 1', columnId: column._id })
    expect(res.status).toBe(201)
    const card = res.body
    ...
    // Update card title
    res = await request(app).patch(`/api/cards/${card._id}`).send({ title: 'Ticket 1 - updated' })
    expect(res.status).toBe(200)

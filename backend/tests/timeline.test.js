...
beforeAll(async () => {
  ...
  Timeline.find = (...args) => mockTimelineFind(...args);
  ...
  MockedTimeline.find = Timeline.find;
...
describe("Timeline API", () => {
  describe("GET /api/timeline", () => {
    it("should return list of timelines", async () => {
      const timelines = [
        {
          _id: "timeline-1",
          boardId: "board-1",
      ...
      mockTimelineFind.mockReturnValue({
        sort: () => ({ lean: () => Promise.resolve(timelines) }),
      });
      ...
      expect(res.body[0]._id).toBe("timeline-1");
    });

...
describe('Timeline API', () => {
  describe('GET /api/timeline', () => {
    it('should return existing snapshot if available', async () => {
      ...
      mockTimelineFindOne.mockReturnValue({ sort: () => ({ lean: () => Promise.resolve(snapshot) }) })
    ...
    it('should compute and persist new snapshot if none exists', async () => {
      mockBoardFindOne.mockResolvedValue({ _id: 'board-1', ownerId: 'owner-1' })
      mockTimelineFindOne.mockReturnValue({ sort: () => ({ lean: () => Promise.resolve(null) }) })
      
      // Mock Sprint and Issue queries for computing timeline
      mockSprintFind.mockReturnValue({ sort: () => ({ lean: () => Promise.resolve([]) }) })
      mockIssueFind.mockReturnValue({ sort: () => ({ lean: () => Promise.resolve([]) }) })
  ...
  describe('POST /api/timeline', () => {
    ...
    it('should create snapshot with provided data', async () => {
      ...
      const customData = { sprints: [{ id: 's1', name: 'Sprint 1' }], backlog: [] }
      const res = await request(app)
        .post('/api/timeline')

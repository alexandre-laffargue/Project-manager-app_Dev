...
async function reorderIssues ({ boardId, sprintId = null, issueId, toPosition = 0 }) {
  ...
  await Promise.all(others.map((it, idx) => {
    if (it.position !== idx) { it.position = idx; return it.save() }
    return null
  }))

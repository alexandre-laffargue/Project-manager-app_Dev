...
async function moveCard ({ boardId, cardId, toColumnId, toPosition = 0 }) {
  ...
  if (sameColumn) {
    const cards = await Card.find({ boardId, columnId: toColumnId }).sort({ position: 1, createdAt: 1 })
  ...
  const fromCards = await Card.find({ boardId, columnId: fromColumnId }).sort({ position: 1, createdAt: 1 })
  const toCards = await Card.find({ boardId, columnId: toColumnId }).sort({ position: 1, createdAt: 1 })

  const fromOthers = fromCards.filter(c => c._id.toString() !== cardId.toString())
  await Promise.all(fromOthers.map((c, i) => {
    if (c.position !== i) { c.position = i; return c.save() }
    return null
  }))

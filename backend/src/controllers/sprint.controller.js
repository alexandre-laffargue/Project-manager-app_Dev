...
const startSprint = async (req, res) => {
  try {
    ...
    if (!sprint) return res.status(404).json({ error: 'Sprint not found' })
    if (sprint.ownerId.toString() !== req.user.sub) { return res.status(403).json({ error: 'Forbidden' }) }
    if (sprint.status === 'active') { return res.status(400).json({ error: 'Sprint already started' }) }
...
const closeSprint = async (req, res) => {
  try {
    ...
    if (!sprint) return res.status(404).json({ error: 'Sprint not found' })
    if (sprint.ownerId.toString() !== req.user.sub) { return res.status(403).json({ error: 'Forbidden' }) }
    if (sprint.status === 'completed') { return res.status(400).json({ error: 'Sprint already closed' }) }
    sprint.status = 'completed'
    // allow optional endDate in body
    if (req.body && req.body.endDate) { sprint.endDate = new Date(req.body.endDate) } else if (!sprint.endDate) sprint.endDate = new Date()
    await sprint.save()
    return res.json(sprint)
  } catch (err) {
    return res.status(500).json({ error: err.message })

...
async function updateTimeline (req, res, next) {
  try {
    ...
    if (req.body.selectedSprints !== undefined) { up.selectedSprints = req.body.selectedSprints }
    if (req.body.selectedIssues !== undefined) { up.selectedIssues = req.body.selectedIssues }
    Object.assign(t, up)
    await t.save()
    res.json({ snapshot: t })
  } catch (err) {
